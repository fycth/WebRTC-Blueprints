    ...
    case M of
        <<"type">> ->
            case Type of
                <<"GETROOM">> ->
                    Room = generate_room(),
                    {Turn_URL,Turn_User,Turn_pass} = get_turn_creds(self()),
                    R = iolist_to_binary(jsonerl:encode({{type, <<"GETROOM">>}, {value, Room}, Turn_URL, Turn_User, Turn_pass})),
                    gproc:reg({p,l, Room}),
                ...
                <<"ENTERROOM">> ->
                    ...
                    case length(Participants) of
                        1 ->
                            ...
                            {Turn_URL,Turn_User,Turn_pass} = get_turn_creds(self()),
                            S = (StateNew#state{room = Room}),
                            R = iolist_to_binary(jsonerl:encode({{type, <<"GETROOM">>}, Turn_URL, Turn_User, Turn_pass})),
                            {reply, {text, <<R/binary>>}, Req, S, hibernate};
                        _ ->
